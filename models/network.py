...
class FCMultiScaleResidualDenseNet(object):
    def __init__(self,
        ...
        self.initializer = tf.keras.initializers.VarianceScaling(scale=2.0, mode='fan_in',
                                                                 distribution='truncated_normal')
        self.regularizer = tf.keras.regularizers.l2(l=1.0)
    ...
    def NetworkForwardPass(self, n_feat_first_layer, n_pool, growth_rate, n_layers_per_block, dropout_rate):
        ...
        tf.summary.image(name='inputs_image', data=inputs, max_outputs=4)
        tf.summary.image('ground_truth', targets[:, :, :, None], max_outputs=4)
        ...
        l = tf.keras.layers.Conv2D(  # Replace tf.layers.conv2d with tf.keras.layers.Conv2D
            ...
            bias_initializer=tf.zeros_initializer(),
            kernel_regularizer=self.regularizer
        )(stack[-1])  # Use the last layer in the stack as input
        stack.append(l)
        ...
        #       Outputs     #
        #####################
        self.logits = tf.keras.layers.Conv2D(filters=self.num_classes,
                                             kernel_size=(1, 1),
                                             padding="same",
                                             activation=None,
                                             kernel_initializer=self.initializer,
        ...
        tf.summary.image('predictions', tf.cast(self.predictions[:, :, :, None], tf.float32), max_outputs=4)

    def Optimize(self):
        update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            trainable_vars = self.trainable_variables  # Use the model's trainable_variables
            ...
            gradients = tape.gradient(self.loss_op, trainable_vars)
            gradients_and_vars = list(zip(gradients, trainable_vars))
    ...
    def Metrics(self):
        for metric_name in self.metrics_list:
            ...
            if metric_name == 'sW_CE_loss':
                ...
                with tf.compat.v1.variable_scope(metric_name):
                    ...
                    tf.summary.scalar('per-step', metric_obj.op)
                    tf.summary.scalar('epoch-avg', metric_obj.avg)
                ...
                with tf.compat.v1.variable_scope(metric_name):
                    ...
                    L2_loss = tf.keras.regularizers.l2(self.weight_decay)(tf.compat.v1.trainable_variables())
                ...
                with tf.compat.v1.variable_scope(metric_name):
                    CE_Loss = tf.reduce_mean(
                        tf.nn.softmax_cross_entropy_with_logits(labels=self.one_hot_labels, logits=self.logits))
