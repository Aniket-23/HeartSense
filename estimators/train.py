...
if __name__ == '__main__':
    conf = conf()
    inputs = tf.keras.Input(shape=(None, None, conf.num_channels), dtype=tf.float32)
    targets = tf.keras.Input(shape=(None, None, None), dtype=tf.uint8)
    weight_maps = tf.keras.Input(shape=(None, None, None), dtype=tf.float32)
    batch_class_weights = tf.keras.Input(shape=(), dtype=tf.float32)
